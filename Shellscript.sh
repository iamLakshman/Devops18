...
USER_ID=$(id -u)
LOG=/tmp/stack.log
TOMCAT_URL=https://www-eu.apache.org/dist/tomcat/tomcat-9/v9.0.27/bin/apache-tomcat-9.0.27.tar.gz
TOMCAT_FILE=$(echo $TOMCAT_URL | cut -d "/" -f9) #echo $TOMCAT_URL | awk -F "/" '{print $NF}'
TOMCAT_FOLDER=$(echo $TOMCAT_FILE | sed -e 's/.tar.gz//')
STUDENT_URL=https://raw.githubusercontent.com/iamLakshman/Devops18/master/student.war
...
cd ../tomcat/apache-tomcat-9.0.27/webapps/
...
cd tomcat/apache-tomcat-9.0.27/bin
